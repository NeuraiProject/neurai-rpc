const $ebb7c8e52e3397d2$export$e13b82b2b0368a6a = {
...
**/ backupwallet: 'backupwallet',
    /** bumpfee has been deprecated on the XNA Wallet.**/ bumpfee: 'bumpfee',
 ...
 */ async function $70bab8e696d427b0$var$requestChallenge(url, authOptions) {
    ...
    try {
        ...
        if (!responseText.startsWith('CHALLENGE|')) throw new Error(`Invalid challenge response: ${responseText}`);
        const parts = responseText.split('|');
        if (parts.length < 3) throw new Error(`Malformed challenge response: ${responseText}`);
        const challenge = parts[1];
        const timeout = parseInt(parts[2], 10);
        if (!challenge || isNaN(timeout)) throw new Error(`Invalid challenge data: ${responseText}`);
 ...
 */ async function $70bab8e696d427b0$var$postDePinData(url, data, challenge, signature) {
    const response = await fetch(url, {
        method: 'POST',
        mode: 'cors',
        headers: {
 ...
 */ async function $70bab8e696d427b0$var$ensureValidChallenge(context, requiredMode) {
    const now = Date.now();
    // Check if we have a valid cached challenge
    if (context.currentChallenge && context.currentChallenge.mode === requiredMode && context.currentChallenge.expiresAt > now) return context.currentChallenge.challenge;
    // Request new challenge
    const authOptions = {
        ...context.authOptions,
        mode: requiredMode
    };
    const challengeData = await $70bab8e696d427b0$var$requestChallenge(context.url, authOptions);
...
function $70bab8e696d427b0$export$e3c4528d9eee3c6(url, authOptions) {
    if (!url) throw new Error('URL is required for DePIN RPC');
    if (!authOptions.token) throw new Error('Token is required for DePIN authentication');
    if (!authOptions.address) throw new Error('Address is required for DePIN authentication');
    if (!authOptions.signMessage || typeof authOptions.signMessage !== 'function') throw new Error('signMessage function is required for DePIN authentication');
    const context = {
        url: url,
        authOptions: authOptions
    };
    return async function depinRpc(method, params) {
        return new Promise(async (resolve, reject)=>{
            try {
                // Determine operation mode from method
                const requiredMode = method === 'depinsendmsg' || method === 'depinsubmitmsg' ? 'SEND' : 'RECEIVE';
                // Get valid challenge
                const challenge = await $70bab8e696d427b0$var$ensureValidChallenge(context, requiredMode);
                ...
                if (!signature) throw new Error('Signature function returned empty result');
                ...
                // Make request
                const response = await $70bab8e696d427b0$var$postDePinData(url, requestData, challenge, signature);
                if (response.ok) {
                    const responseData = await response.json();
                    if (responseData.error) {
                        // Check if it's an expired challenge error
                        if (responseData.error.message && responseData.error.message.includes('expired')) {
                    ...
                    } else resolve(responseData.result);
                } else {
...
async function $70bab8e696d427b0$export$9d7d174baf084291(url, authOptions) {
    return $70bab8e696d427b0$var$requestChallenge(url, authOptions);
}


function $c3f6c693698dc7cd$var$throwSyntaxError() {
    throw new Error("Syntax error, call getRPC with (username, password, URL)");
}
function $c3f6c693698dc7cd$export$eb94c228b080d168(username, password, URL) {
    if (!username) $c3f6c693698dc7cd$var$throwSyntaxError();
    if (!password) $c3f6c693698dc7cd$var$throwSyntaxError();
    if (!URL) $c3f6c693698dc7cd$var$throwSyntaxError();
    return async function rpc(method, params) {
        const promise = new Promise((resolutionFunc, rejectionFunc)=>{
            ...
            try {
                const rpcResponse = $c3f6c693698dc7cd$var$postData(URL, data, username, password);
            ...
            */ if (response.ok) {
                    ...
                    rejectionFunc({
                        ...
                        description: "Are you sure that the URL is correct? The URL is usually mainnet = http://127.0.0.1:19001 and testnet =  http://127.0.0.1:19101"
                    });
                });
            } catch (e) {
...
function $c3f6c693698dc7cd$var$getValidMethods() {
    const keys = Object.keys((0, $ebb7c8e52e3397d2$export$e13b82b2b0368a6a)).sort();
    return keys.join(" ");
}
// Example POST method implementation:
async function $c3f6c693698dc7cd$var$postData(url = "", data = {}, username, password) {
    ...
    else if (Buffer) base64Credentials = Buffer.from(`${username}:${password}`).toString("base64");
...
export {$c3f6c693698dc7cd$export$eb94c228b080d168 as getRPC, $ebb7c8e52e3397d2$export$e13b82b2b0368a6a as methods, $70bab8e696d427b0$export$e3c4528d9eee3c6 as getDePinRPC, $70bab8e696d427b0$export$9d7d174baf084291 as requestDePinChallenge};
